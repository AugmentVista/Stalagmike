...
public class EnemyStateMachine : MonoBehaviour
{
    ...
    void PatrolState()
    {
        ...
        if (Vector2.Distance(Self.position, targetPoint) <= 5.0f)
        {
            switch (targetswitch)
            {
                case 0:
    ...
    void AttackState()
    {
        ...
        if (inRangeOfPlayer())
        {
            ...
            if (elapsedTime >= interactionDelay && AttackSound != null)
            {
                ...
                //Player.rb.AddForce(Vector2.forward * EnemyPower, ForceMode.Impulse);
                elapsedTime = 0;
                numberOfAttacks++;
            }
        }
    ...
    void RetreatState()
    {
        ...
        if (Vector2.Distance(Self.position, Safety) <= 1.5f)
        {
            if (CanSeePlayer())
            {
                currentState = State.Chase;
    ...
    {
        if (Vector2.Distance(transform.position, player.position) <= sensorRange)
        {
            RaycastHit hit;
    ...
    {
        float distanceToTarget0 = Vector2.Distance(player.position, targetPoint0);
        float distanceToTarget1 = Vector2.Distance(player.position, targetPoint1);
        float distanceToTarget2 = Vector2.Distance(player.position, targetPoint2);
        float distanceToTarget3 = Vector2.Distance(player.position, targetPoint3);
        float distanceToTarget4 = Vector2.Distance(player.position, targetPoint4);
